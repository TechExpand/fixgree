...
class WebServices extends ChangeNotifier {
    ...
    Future<dynamic> Register({context, scaffoldKey}) async {
        ...
        try {
            var response = await http
                ...
                'mobile': data.number.toString().substring(data.number.dialCode.length, data.number.toString().length),
                'firstName': data.firstName.toString(),
                'lastName': data.lastName.toString(),
                'device_token': data.firebase_user_id.toString()??'',
                'device_os': 'Andriod',
                'device_type': 'phone',
                'email': data.emails.toString() == null || data.emails.toString().isEmpty
    ...
    Future<dynamic> NearbyArtisans({longitude, latitude}) async {
        var response = await http
            .post(Uri.parse('https://manager.fixme.ng/near-artisans'),
            body: {
    ...
    Future Search({longitude, latitude, searchquery}) async {
        try {
            var response = await http
                .post(Uri.parse('https://manager.fixme.ng/search-artisans'),
                body: {
                    'user_id': user_id.toString(),
    ...
    Future<dynamic> initiateTransfer(
        {bankCode, accountNumber, accountName, amount, secPin, naration}) async {
        var response = await http
            .post(Uri.parse(' https://manager.fixme.ng/initiate-transfer'), body: {
            'user_id': user_id.toString(),
            'bankCode': bankCode,
